...
public class Tether : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        numOfKnots = (int) Mathf.Floor(distance/0.2f);

        if (numOfKnots > transform.childCount) {
            addKnots(transform.childCount - numOfKnots);
        } else if (transform.childCount < numOfKnots) {
            removeKnots(numOfKnots - transform.childCount);
        }
    }
